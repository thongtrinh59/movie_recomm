...
class Movieshows extends Component{
  ...
  render(){
    console.log(this.props.messages);
    var tuiti = this.props.messages.at(-1)["message"];
    var temp1 = tuiti.split("****")
    var temp2 = temp1.map(item => item.split("++++"))
      ...
      <div class='moviescrollbar'> 
        {
          temp2.map(ii => {
            return (
              <a href={'https://www.imdb.com/title/' + ii[1]} target='_blank'>
                <div class='movie-box'>
                  <img src={'http://img.omdbapi.com/?apikey=ea75cc5f&i=' + ii[1]}/>
                  <p class='movie-title'>{ii[0]}</p>
                  <p class='movie-details'>{ii[2]} • {ii[3]} • {ii[4]}</p>
                </div>
              </a>
            );
          })
